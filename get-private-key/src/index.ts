...
export function getPrivateKey(options: Options = {}): string | null {
  ...
  if (env.PRIVATE_KEY_PATH) {
    ...
    if (existsSync(filepath)) {
      ...
      throw new Error(
        `[@octotask/get-private-key] Private key does not exists at path: "${env.PRIVATE_KEY_PATH}". Please check to ensure that "env.PRIVATE_KEY_PATH" is correct.`,
      );
    }
  }
  ...
  if (pemFiles.length > 1) {
    ...
    throw new Error(
      `[@octotask/get-private-key] More than one file found: "${paths}". Set { filepath } option or set one of the environment variables: PRIVATE_KEY, PRIVATE_KEY_PATH`,
    );
  } else if (pemFiles[0]) {
