    ...
    describe("PKCS1 format", () => {
      ...
      it("two *.pem files in current working directory", () => {
        ...
        expect(() => getPrivateKey()).toThrow(
          `[@octotask/get-private-key] More than one file found: \"test1.pem, test2.pem\". Set { filepath } option or set one of the environment variables: PRIVATE_KEY, PRIVATE_KEY_PATH`,
        );
      });
  ...
  describe("with environment variables", () => {
    describe("PKCS1 format", () => {
      ...
      it("PRIVATE_KEY invalid", () => {
        ...
        expect(() => getPrivateKey()).toThrow(
          `[@octotask/get-private-key] The contents of "env.PRIVATE_KEY" could not be validated. Please check to ensure you have copied the contents of the .pem file correctly.`,
        );
      });
      ...
      it("PRIVATE_KEY_PATH does not exist", () => {
        ...
        expect(() => getPrivateKey()).toThrow(
          `[@octotask/get-private-key] Private key does not exists at path: "test.pem". Please check to ensure that "env.PRIVATE_KEY_PATH" is correct.`,
        );
      });
