...
describe("README examples", () => {
  ...
  describe("Get authenticated octokit instance based on event", () => {
    test("with token auth", async () => {
      ...
      })) as unknown as InstanceType<typeof OctotaskOctokit>;
    ...
    test("with app auth and push event", async () => {
      ...
      const OctotaskOctokitWithRequestMock = OctotaskOctokit.defaults({
        request: {
          fetch: mock,
        },
      });
      const octokit = new OctotaskOctokitWithRequestMock({
        auth: {
          appId: APP_ID,
          privateKey: PRIVATE_KEY,
        },
