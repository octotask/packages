...
export async function getConfigFiles(
  ...
  do {
    ...
    if (alreadyLoaded) {
      throw new Error(
        `[@octotask/octokit-plugin-config] Recursion detected. Ignoring  "_extends: ${extendRepoConfig.config._extends}" from ${extendRepoConfig.url} because ${alreadyLoaded.url} was already loaded.`,
      );
    }
  } while (true);
}
