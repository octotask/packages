...
export async function getConfigFile(
  ...
  if (!SUPPORTED_FILE_EXTENSIONS.includes(fileExtension)) {
    throw new Error(
      `[@octotask/octokit-plugin-config] .${fileExtension} extension is not support for configuration (path: "${path}")`,
    );
  }
  ...
  try {
    ...
    if (headers["content-type"] === "application/json; charset=utf-8") {
      throw new Error(
        `[@octotask/octokit-plugin-config] ${url} exists, but is either a directory or a submodule. Ignoring.`,
      );
    }
    ...
    if (fileExtension === "json") {
      if (typeof data === "string") {
        throw new Error(
          `[@octotask/octokit-plugin-config] Configuration could not be parsed from ${url} (invalid JSON)`,
        );
      }
    ...
    if (typeof config === "string") {
      throw new Error(
        `[@octotask/octokit-plugin-config] Configuration could not be parsed from ${url} (YAML is not an object)`,
      );
    }
    ...
    if (error.name === "YAMLException") {
      ...
      throw new Error(
        `[@octotask/octokit-plugin-config] Configuration could not be parsed from ${url} (${reason})`,
      );
    }
