const { Context, OctotaskOctokit } = require('octotask')
    ...
    describe('set', () => {
      test('sets new metadata', async () => {
        const mock = nock('https://api.github.com')
          ...
          .patch('/repos/foo/bar/issues/42', (requestBody) => {
            expect(requestBody.body).toEqual('original post\n\n<!-- octotask = {"1":{"key":"value","hello":"world"}} -->')
            return true
          })
          .reply(204)
    ...
    describe('set', () => {
      test('sets new metadata', async () => {
        const mock = nock('https://api.github.com')
          .get('/repos/foo/bar/issues/42')
          .reply(200, {
            body: 'original post\n\n<!-- octotask = {"2":{"key":"value"}} -->'
          })
          .patch('/repos/foo/bar/issues/42', (requestBody) => {
            expect(requestBody.body).toEqual('original post\n\n<!-- octotask = {"1":{"hello":"world"},"2":{"key":"value"}} -->')
            return true
          })
          .reply(204)
